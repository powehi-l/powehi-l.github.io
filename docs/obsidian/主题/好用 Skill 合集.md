...
个人用过的、觉得很好用的 Agent Skills 记录。方便日后回顾与推荐。

## find-skills

- **一句话**：帮你发现并安装开源 agent skills 的入口。
- **项目**：[vercel-labs/skills](https://github.com/vercel-labs/skills)（find-skills）
- **核心**：以 `npx skills` 为包管理，`find`（搜索）→ `add`（安装）→ `update`（更新），配合 [skills.sh](https://skills.sh/) 排行榜挑选。
- **我为什么记它**：安装生态里几乎所有 skill 都从这入口开始，是"找 skill"的敲门砖。已安装 ✅
